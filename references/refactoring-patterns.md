...
## Pattern 6: Mass Rename/Replace

**When**: Need to rename a prop, variable, or function used 10+ times throughout a file

**Steps**:
1. Identify all usages of the name to be changed
2. Verify the rename is purely textual (no logic changes)
3. Execute as ONE atomic change using find-and-replace
...
```jsx
// Before: "userId" used 15 times throughout component
function UserProfile({ userId }) {
  const [user, setUser] = useState(null);
...
// After: Single atomic change - rename "userId" to "id" everywhere
...
**Critical Rules**:
- Must be purely textual substitution (no logic changes)
- All occurrences must be renamed in same atomic change
- Use global find-and-replace (replace_all: true) to ensure consistency
- Acceptable even if 40+ lines affected (still one atomic concern)
- If rename requires logic changes, split into phases

**Common Mistakes**:
- ❌ Renaming only some occurrences (causes undefined variable errors)
- ❌ Combining rename with logic changes
- ❌ Not testing after rename (may have missed prop destructuring, etc.)
...
7. **External dependencies**: Requires simultaneous modification of external API contracts (GraphQL queries, REST endpoints, TypeScript interfaces/types shared across files) AND component logic - split into separate atomic changes
